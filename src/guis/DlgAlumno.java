...
public class DlgAlumno extends JDialog implements ActionListener, KeyListener, MouseListener  {
	...
	public DlgAlumno() {
		...
		setIconImage(Toolkit.getDefaultToolkit().getImage(DlgAlumno.class.getResource("/img/SAN.png")));
		...
		panel.setBounds(487, 52, 164, 149);
		...
		panel_1.setBounds(10, 23, 341, 234);
		...
		lblNewLabel.setIcon(new ImageIcon(DlgAlumno.class.getResource("/img/WhatsApp Image 2021-11-29 at 12.09.48 AM.jpeg")));
		lblNewLabel.setBounds(380, 73, 97, 161);
		getContentPane().add(lblNewLabel);
		{
			label = new Label("FILTRAR:");
			label.setBounds(494, 235, 62, 22);
		...
		
		{
			...
			txtFiltrar.setBounds(565, 235, 86, 20);
	...
	protected void BtnModificar(ActionEvent arg0) {
		...
		try{
			if (aa.tamano() == 0) {
				...
				habilitarEntradas(false);
				mensaje("No existe registro de alumnos");	
			}
			else{
				habilitarBotones(true);
				habilitarEntradas(false);
				...
				String dni = leerDNIAlumno();
					try{
						...
						try{
							...
							error("Ingrese N� de celular del Alumno", txtCelular);
						}
					}
					catch(Exception e){
					...
					error("Ingrese el N� de DNI del Alumno", txtDNI);
				}
			}
			else
	...
	public void keyReleased(KeyEvent arg0) {	
		 if (arg0.getSource() == txtFiltrar) {
		        keyTypedTxtFiltrar(arg0);
		    }
	...
	void listar(){
		...
		if (modelo.getRowCount() == aa.tamano() - 1)
			posFila = aa.tamano() - 1;
		if (posFila == aa.tamano())
			posFila --;
	...
	//M�todos que validan los caracteres ingresados-Codigo ASII
	...
	// Implementación del método keyTypedTxtFiltrar
	protected void keyTypedTxtFiltrar(KeyEvent arg0) {
	    filtrarTabla(txtFiltrar.getText());
	}
}
